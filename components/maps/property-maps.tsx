...
const MapContent = dynamic(
  () => import("@/components/maps/map-content"),
  { ssr: false }
);
...
export function PropertyMapSystem({
  ...
  const [mapProvider, setMapProvider] = useState<"LEAFLET" | "STATIC">("LEAFLET");

  const analytics = neighborhoodInsights || {
    walkScore: 89,
  ...
  return (
    <div className="bg-[#0D0E12] border border-white/5 rounded-2xl overflow-hidden shadow-xl">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 min-h-[340px] bg-[#111318] relative flex items-center justify-center border-b lg:border-b-0 lg:border-r border-white/5">
          {mapProvider === "LEAFLET" ? (
            <div className="absolute inset-0 w-full h-full">
              <MapContent lat={lat} lng={lng} propertyName={propertyName} />
            </div>
          ) : (
        ...
        {/* Right Side - Neighborhood Analytics */}
