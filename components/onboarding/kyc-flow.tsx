        ...
        <div className="flex items-center justify-between mb-4">
          {steps.map((step, i) => (
            <div key={step.id} className="flex items-center">
              <div
                className={`w-10 h-10 rounded-full flex items-center justify-center ${i <= currentStep ? "gradient-gold text-white" : "bg-muted text-muted-foreground"}`}
              >
                <step.icon className="w-5 h-5" />
              </div>
              {i < steps.length - 1 && (
                <div
                  className={`w-16 h-0.5 mx-2 ${i < currentStep ? "bg-gold-500" : "bg-muted"}`}
                />
              )}
            </div>
          ))}
        ...
        >
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="border-2 border-dashed border-border/50 rounded-xl p-8 text-center hover:border-gold-500/50 transition-colors cursor-pointer">
                  ...
                  <p className="text-sm font-medium">Upload ID Front</p>
                ...
                <div className="border-2 border-dashed border-border/50 rounded-xl p-8 text-center hover:border-gold-500/50 transition-colors cursor-pointer">
                  <Upload className="w-8 h-8 mx-auto mb-2 text-muted-foreground" />
                  <p className="text-sm font-medium">Upload ID Back</p>
                  <p className="text-xs text-muted-foreground">
                    PNG, JPG up to 5MB
                  </p>
                </div>
          ...
          {currentStep === 2 && (
            <div className="text-center space-y-4">
              ...
              <p className="text-sm text-muted-foreground">
                Take a clear selfie holding your ID document. Ensure good
          ...
          {currentStep === 3 && (
            <div className="space-y-4">
              <div className="flex items-center gap-3 p-4 rounded-lg bg-green-500/10 border border-green-500/20">
                ...
                <div>
                  <p className="font-medium text-green-500">Ready to Submit</p>
                  <p className="text-sm text-muted-foreground">
                    Your information will be reviewed within 24 hours.
                  </p>
                </div>
              </div>
              <div className="space-y-2">
                ...
                <div className="p-4 rounded-lg bg-background/50 space-y-1 text-sm">
                  ...
                  <p>
                    ...
                    {formData.idType.replace("_", " ").toUpperCase()} ****
